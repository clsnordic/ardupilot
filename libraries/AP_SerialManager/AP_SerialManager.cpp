...
const AP_Param::GroupInfo AP_SerialManager::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("1_PROTOCOL",  1, AP_SerialManager, state[1].protocol, SerialProtocol_Ext_Nav),
    ...
    // @User: Standard
    AP_GROUPINFO("1_BAUD", 2, AP_SerialManager, state[1].baud, AP_SERIALMANAGER_EXT_NAV_BAUD/1000),
        ...
        if (state[i].uart != nullptr) {
            switch (state[i].protocol) {
                ...
                case SerialProtocol_MAVLink2:
                    ...
                    break;
                case SerialProtocol_Ext_Nav:
                    state[i].uart->begin(map_baudrate(state[i].baud),
                                         AP_SERIALMANAGER_EXT_NAV_BUFSIZE_RX,
                                         AP_SERIALMANAGER_EXT_NAV_BUFSIZE_TX);
                    break;
